...
# DE4DD - Dark Escape 4D Security Dongle Emulator, made by dazzaXx.

set -euo pipefail

USBIMG="/DE4DD/DE4DD_USB.img"
SHLOG="/DE4DD/DE4DD.log"
LIBCOMPOSITE="/etc/modules-load.d/libcomposite.conf"
BOOTCONFIG="/boot/config.txt"
SERVICE="/etc/systemd/system/DE4DD.service"
timestamp=$(date)

if [ -f "$SHLOG" ]; then # Check if the log exists, if it does then delete it and print the log header.
	rm /DE4DD/DE4DD.log
	echo "Dark Escape 4D DONGLE EMULATOR LOG" > "$SHLOG"
	echo -e "-------------------------------------------------\n" >> "$SHLOG"
fi
...
if [ ! -f "$SERVICE" ]; then # Check if the T6BR service doesnt exist, if not, create it.
	...
	echo -e "[Unit]\nDescription=DE4DD - Dead Escape 4D Dongle Emulator\n\n[Service]\nExecStart=/DE4DD/DE4DD.sh\n\n[Install]\nWantedBy=multi-user.target" > "$SERVICE"
	systemctl enable DE4DD # Enable the service and start it.
	systemctl start DE4DD
else
...
if [ ! -f "$USBIMG" ]; then # Check if the emulated USB's storage image exists, if not, create a 265MB .IMG and format it to FAT32.
	echo "[$timestamp] Dark Escape 4D USB Image DOES NOT exist, creating blank image." >> "$SHLOG"
	dd if=/dev/zero of=/DE4DD/DE4DD_USB.img bs=1M count=265
	echo "[$timestamp] Blank Image Generated." >> "$SHLOG"
	mkdosfs -F 32 /DE4DD/DE4DD_USB.img
	echo "[$timestamp] Blank Image formatted to FAT32." >> "$SHLOG"
else
	echo "[$timestamp] Dark Escape 4D USB Image already exists, skipping." >> "$SHLOG"
fi
...
mkdir -p /sys/kernel/config/usb_gadget/DE4DD_Dongle
cd /sys/kernel/config/usb_gadget/DE4DD_Dongle
echo 0x0b9a > idVendor
echo 0x0c00 > idProduct
echo 0x0100 > bcdDevice
echo 0x0200 > bcdUSB
mkdir -p strings/0x409
echo "268611070717" > strings/0x409/serialnumber
echo "dazzaXx" > strings/0x409/manufacturer
echo "Dark Escape 4D Dongle Emulator" > strings/0x409/product
