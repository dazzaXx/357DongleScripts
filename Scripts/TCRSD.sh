...
# TCRSD - Time Crisis Razing Storm Security Dongle Emulator, made by dazzaXx.

set -euo pipefail

USBIMG="/TCRSD/TCRSD_USB.img"
SHLOG="/TCRSD/TCRSD.log"
LIBCOMPOSITE="/etc/modules-load.d/libcomposite.conf"
BOOTCONFIG="/boot/config.txt"
SERVICE="/etc/systemd/system/TCRSD.service"
timestamp=$(date)

if [ -f "$SHLOG" ]; then # Check if the log exists, if it does then delete it and print the log header.
	rm /TCRSD/TCRSD.log
	echo "Time Crisis Razing Storm DONGLE EMULATOR LOG" > "$SHLOG"
	echo -e "-------------------------------------------------\n" >> "$SHLOG"
fi
...
if [ ! -f "$SERVICE" ]; then # Check if the T6BR service doesnt exist, if not, create it.
	...
	echo -e "[Unit]\nDescription=TCRSD - Tekken 6 BR Dongle Emulator\n\n[Service]\nExecStart=/TCRSD/TCRSD.sh\n\n[Install]\nWantedBy=multi-user.target" > "$SERVICE"
	systemctl enable TCRSD # Enable the service and start it.
	systemctl start TCRSD
else
...
if [ ! -f "$USBIMG" ]; then # Check if the emulated USB's storage image exists, if not, create a 265MB .IMG and format it to FAT32.
	echo "[$timestamp] Tekken 6 BR USB Image DOES NOT exist, creating blank image." >> "$SHLOG"
	dd if=/dev/zero of=/TCRSD/TCRSD_USB.img bs=1M count=265
	echo "[$timestamp] Blank Image Generated." >> "$SHLOG"
	mkdosfs -F 32 /TCRSD/TCRSD_USB.img
	echo "[$timestamp] Blank Image formatted to FAT32." >> "$SHLOG"
else
	echo "[$timestamp] Time Crisis Razing Storm USB Image already exists, skipping." >> "$SHLOG"
fi
...
mkdir -p /sys/kernel/config/usb_gadget/TCRS_Dongle
cd /sys/kernel/config/usb_gadget/TCRS_Dongle
echo 0x0693 > idVendor
echo 0x0026 > idProduct
echo 0x0100 > bcdDevice
echo 0x0200 > bcdUSB
mkdir -p strings/0x409
echo "026391812730" > strings/0x409/serialnumber
echo "dazzaXx" > strings/0x409/manufacturer
echo "Time Crisis RS Dongle Emulator" > strings/0x409/product
